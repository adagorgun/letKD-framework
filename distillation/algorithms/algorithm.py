...
import distillation.criterions as criterions
...
class Algorithm:
    ...
    def init_all_networks(self):
        ...
        for key, val in networks_defs.items():
            ...
            if key=='student_net':
                ...
                if (key == 'hint_layer_connection') :
                    net_opt = self.arrange_config(net_opt)
    ...
    def arrange_config(self, net_opt):
        
        extract_layer_t = net_opt['extract_layer'] #self.opt['networks']['teacher_net']['opt']['extract_from']
        model_t = self.networks['feature_extractor_target']
        
        extract_layer_s = net_opt['extract_layer']  #self.opt['networks']['student_net']['opt']['extract_from']
        model_s = self.networks['feature_extractor']
        
        split_model_t, _ = split_network(model_t, extract_layer_t, [False])
        split_model_s, _ = split_network(model_s, extract_layer_s, [False])
        ...
        feat_t = split_model_t(data)
        feat_s = split_model_s(data)
        
        net_opt['s_shape'] = feat_s.shape
        net_opt['t_shape'] = feat_t.shape
    ...
    def init_criterion(self, ctype, copt):
        if (ctype == 'KD') or (ctype == 'AT') or (ctype == 'hint_3x3') or (ctype == 'hint_3x3_withClassifier') or (ctype == 'subclass_loss'):
            return criterions.factory(criterion_name=ctype, opt=copt)
